<script lang="ts">
	...
	import type { ReturnObject } from './submitMutation';
	import { createForm } from 'svelte-forms-lib';
	import * as yup from 'yup';
	import { mutate } from './submitMutation';
	...
	let formStatus = writable<ReturnObject>();

	const onSubmit = async (values: any) => {
		console.log('form submit');
		//alert(JSON.stringify(values));
		console.log('values', values);
		$formStatus = await mutate(values);
	};
...
<div class="container">
	<h1>Get a website <a href="/moreinfo">for free</a></h1>
	<form on:submit={handleSubmit}>
		<div class="thisForm">
			...
			<div>
				<div class="leftbox">
					<label for="project_type" value="">*Choose project type</label>
				</div>
				<div class="rightbox">
					<div class="select">
						<select
			...
			<div>
				<button class="submitbutton" type="submit" disabled={!$isValid}
					>{#if $isSubmitting}loading...{:else}Submit{/if}</button
				>
			</div>
			{#if $formStatus}
				<p>
					Hi {$formStatus.name}, we have recieved your reply! &#128512;
				</p>
			{/if}
		</div>
	</form>
